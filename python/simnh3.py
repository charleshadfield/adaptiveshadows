...
def main():
    ...
    ### Ground information
    print("Retrieving precomputed ground energy and ground state...")
    t0 = time.time()
    with open('nh3_ground/{}.npy'.format(name), 'rb') as f:
        ground_energy = np.load(f)
        ground_state = np.load(f)
    t1 = time.time()
    diff = int(t1-t0)
    print("Done\nGround information retrieved in {}min{}sec.".format(diff // 60, diff % 60))
